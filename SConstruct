...
VARS.AddVariables(
    ...
    EnumVariable("imageFS",
                 help="Type of filesystem",
                 default="fat32",
                 allowed_values=("fat12", "fat16", "fat32", "ext2"))
)

VARS.Add("imageSize",
         help="The size of the image which will be rounded up to the nearest multiple of 512." +
              "You can use suffixes (k, M, G). " +
              "For floppy disks, the size is fixed to 1.44MB.",
         default="250M",
         converter=ParseSize)
VARS.Add("toolchain",
         help="Path to toolchain dir.",
         default="toolchain")
